...
<script>
    $(document).ready(function() {
        function loadProfile() {
            $.ajax({
                url: 'function/loadProfile.php',
                method: 'POST',
                data: {
                    loadProfile: 1,
                },
                success: function(data) {
                    $(".profile-wraper").html(data);
                }
            })
        }
        loadProfile();
        $(document).on("click", ".edit-profile", function() {
            var btn = $(this);
            var HovaTen = $(".input-info");
            if (btn.text() == "Edit") {
                HovaTen.prop("disabled", false);
                btn.text("Done");
            } else {
                HovaTen.prop("disabled", true);
                btn.text("Edit");
            }
        })

        $(document).on("click", ".submit-profile", function() {
            var tenkh = $("#input-kh").val();
            var email = $("#input-email").val();
            var sdt = $("#input-sdt").val();
            var diachi = $("#input-diachi").val();
            var gioitinh = $("#input-gioitinh").val();
